...
def a101_get_data(search_product):
    print("")
    # utf8_search_words = "meyve"
    utf8_search_words = search_product
    ascii_search_words = urllib.parse.quote(utf8_search_words)
    url ="https://www.a101.com.tr/list/?search_text=" + ascii_search_words
    print(url)
    ...
    try:
        pages_all = soup.find('nav', {'class' : 'pagination'})
        pages = pages_all.find_all('a', {'class' : 'page-link js-pagination'})
        count = 0
        for item in pages :
            if item.text:
                print("A101 search and pages : " + item.text.strip())
                count=count+1
    except Exception as e :
        print('Try Except error meesage, A101 count pages : ' , e)    

    all_divs = soup.find('ul', {'class' : 'product-list-general'})
    products = all_divs.find_all('h3', {'class' : 'name'})
    prices = all_divs.find_all('span', {'class' : 'current'})
    ...
    for item in products :
        ...
        count=count+1
        # .strip() remove spaces
        list1.append([count, "A101", item.text.strip()]) 
...
# a101_data_frame = a101_get_data("ekmek")
