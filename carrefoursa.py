...
# url ="https://www.carrefoursa.com/search/?text=k%C4%B1yma"
# url ="https://www.carrefoursa.com/search/?text=ekmek"


def carrefoursa_get_data(search_product):
    print("")
    # utf8_search_words = "domates salça"
    utf8_search_words = search_product
    ascii_search_words = urllib.parse.quote(utf8_search_words)
    url ="https://www.carrefoursa.com/search/?text=" + ascii_search_words
    print(url)
    ...
    try:
        pages_all = soup.find('div', {'class' : 'pagination-container'}).find('div', {'class' : 'owl-stage'})
        pages = pages_all.find_all('div', {'class' : 'owl-item'})
        count = 0
        for item in pages :
            if item.text:
                print("Carrefoursa search and pages : " + item.text.strip())
                count=count+1
    except Exception as e :
        print('Try Except error meesage, Carrefoursa count pages : ' , e)    


    all_divs = soup.find('div', {'class' : 'pl-grid-cont row'})
    products = all_divs.find_all('span', {'class' : 'item-name'})
    prices = all_divs.find_all('span', {'class' : 'item-price js-variant-discounted-price'})
    ...
    for item in products :
        # print(item.text)
        count=count+1
        list1.append([count, "Carrefoursa", item.text])
...
# carrefoursa_data_frame = carrefoursa_get_data("ekmek")
